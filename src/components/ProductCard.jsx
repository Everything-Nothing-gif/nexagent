...
export default function ProductCard({ product, wallet, onPurchase }) {
  ...
  return (
    <div style={{
      ...
      <div style={{fontSize:10, fontFamily:'DM Mono, monospace', color:'#55556a', textTransform:'uppercase', letterSpacing:'0.08em', marginTop: b ? 6 : 0}}>
        {product.store}
      </div>
      ...
      {/* Price row — USD large + ALGO inline beside it */}
      <div style={{display:'flex', alignItems:'center', gap:8, marginTop:2, flexWrap:'wrap'}}>
        ...
        <span style={{
          ...
          color: b ? 'rgba(29,158,117,0.85)' : '#3a3a52',
          textShadow: b ? '0 0 10px rgba(29,158,117,0.55)' : 'none',
        }}>
          ≈ {toAlgoPrice(product.price)} ALGO
        </span>
      </div>

      <div style={{fontSize:11, fontFamily:'DM Mono, monospace', color: b ? '#e8b84b' : '#55556a', lineHeight:1.6}}>
        {'★'.repeat(Math.round(product.rating))}{'☆'.repeat(5-Math.round(product.rating))}
        {' '}<span style={{color: b ? '#9090a8' : '#45455a'}}>{product.rating} · {product.reviews.toLocaleString()} reviews</span>
      </div>

      <div style={{fontSize:11, fontFamily:'DM Mono, monospace', color:'#45455a', lineHeight:1.6}}>
      ...
      {txId && (
        <a href={`https://testnet.algoexplorer.io/tx/${txId}`} target="_blank" rel="noopener noreferrer"
          style={{fontSize:10, fontFamily:'DM Mono, monospace', color:'#1d9e75', textDecoration:'none', textAlign:'center', marginTop:4}}>
          View on explorer ↗
        </a>
      )}
    </div>
